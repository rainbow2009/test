...
abstract class BaseModelMethods
{
    ...
    final protected function createFields($set, $table = false, $join = false)
    {
        ...
        if (!isset($set['fields']) || !is_array($set['fields']) || !$set['fields']) {
            ...
            foreach ($set['fields'] as $field) {
                ...
                if ($join_structure && !$id_fields && $this->tableRows[$table] === $field) {
                    $id_fields = true;
                }
    ...
    final protected function createOrder($set, $table = false)
    {
        ...
        if (isset($set['order']) && $set['order']) {
            ...
            foreach ($set['order'] as $order) {
                ...
                if (in_array($order, $this->sqlFunct)) {
                    $order_by .= $order . ',';
                } elseif (is_int($order)) {
    ...
    final protected function createJoin($set, $table, $new_where = false)
    {
        ...
        if ($set['join']) {
            ...
            foreach ($set['join'] as $key => $value) {
                ...
                $concatTable = $this->createTableAlias($table);
                ...
                if (isset($value['on']) && $value['on']) {
                    ...
                    if (isset($value['on']['fields']) && is_array($value['on']['fields'])
                        && count($value['on']['fields']) == 2) {
                    ...
                    $join .= $this->createTableAlias($join_temp_table);
